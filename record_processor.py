    ...
    def validate_mapping(self, results: list[MaskResult]) -> bool:
        ...
        for res in sorted_results:
            ...
            if res.start > current_pos_in_text:
                reconstructed_parts.append(self.text_record[current_pos_in_text : res.start])
        ...
        if current_pos_in_text < len(self.text_record):
            reconstructed_parts.append(self.text_record[current_pos_in_text:])
        # Handle case where text_record might be shorter than expected based on result ends
        elif current_pos_in_text > len(self.text_record):
            print(f"Validation Warning: Result end index {current_pos_in_text} exceeds text_record length {len(self.text_record)}. Validation may fail.")


        reconstructed_res = "".join(reconstructed_parts)

        # Perform the comparison
        is_valid = reconstructed_res == self.res_record
